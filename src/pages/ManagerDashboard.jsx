  ...
  return (
    <div>
      <header style={{ background: 'white', padding: '15px 0', borderBottom: '2px solid #d4af37', position: 'sticky', top: 0, zIndex: 100 }}>
        {/* RESPONSIVIDADE: flex-wrap permite quebrar linha no celular */}
        <div className="container header-content" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center', marginTop: '5px' }}>
            ...
            <button className="btn btn-outline" style={{ padding: '8px 12px' }} onClick={handleLogout}><LogOut size={16}/> Sair</button>
          </div>
        </div>
      </header>

      <div className="container" style={{ paddingTop: '30px' }}>
        {/* RESPONSIVIDADE: Botões empilhados ou lado a lado dependendo do espaço */}
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '30px', justifyContent: 'center' }}>
          ...
          <button className="btn btn-danger" onClick={resetAll}><Trash size={18}/> Reset</button>
        </div>
      ...
      {activeModal === 'notifications' && (
        <Modal title="Notificações" onClose={() => setActiveModal(null)}>
          ...
          <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
            {!notifications || notifications.length === 0 ? <p style={{textAlign: 'center', color: '#94a3b8'}}>Nenhuma notificação.</p> : notifications.map(n => (
      ...
      {activeModal === 'car' && (
        <Modal title={carForm.id ? "Editar" : "Cadastrar"} onClose={() => setActiveModal(null)}>
          <form onSubmit={handleCarSubmit}>
            ...
            <div style={{ width: '100%', border: '2px dashed #cbd5e1', borderRadius: '8px', padding: '10px', textAlign: 'center', marginBottom: '15px' }} onClick={() => document.getElementById('file-upload').click()}>
               {carForm.image ? "Imagem Carregada (Clique para trocar)" : "Clique para enviar foto"}
            </div>
      ...
      {activeModal === 'client' && (
         <Modal title="Cliente" onClose={() => setActiveModal(null)}>
           <form onSubmit={handleClientSubmit}>
             <input placeholder="Nome" required value={clientForm.name} onChange={e => setClientForm({...clientForm, name: e.target.value})} />
             <input placeholder="CPF" required value={clientForm.cpf} onChange={e => setClientForm({...clientForm, cpf: e.target.value})} />
             <input placeholder="Email" required value={clientForm.email} onChange={e => setClientForm({...clientForm, email: e.target.value})} />
             <input placeholder="Telefone" required value={clientForm.phone} onChange={e => setClientForm({...clientForm, phone: e.target.value})} />
             <button type="submit" className="btn btn-primary" style={{width:'100%'}}>Salvar</button>
           </form>
         </Modal>
      )}
